...
- **玩家名字無法預錄**（是動態文字），所以「換○○囉！」「恭喜○○獲勝」永遠用裝置 TTS。
...
| 設定 | 影響範圍 |
|---|---|
| **旁白模式** | `recorded`（預設）題目與回饋用錄音；`device` 全部改用裝置 TTS，讓不喜歡錄音聲音的人能自選聲音 |
| **語速** | **兩邊都套用**——錄音用 `playbackRate`（0.85／1／1.2，音檔本身已 -10%）、TTS 用 `rate`（0.8／0.95／1.1），兩組數值經過校準聽感相近 |
| **中文／英文語音** | `device` 模式下套用到全部；`recorded` 模式下只影響唸名字的句子（對話框內會即時顯示目前是哪種情況） |

錄音播放一律設 `preservesPitch`，慢速時不會變低沉。
